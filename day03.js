...
let exampleSolution1 = 157;

let exampleSolution2 = 70;
...
const priority = '_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('');
let solution1 = input.reduce((sum, curr) => {
    const [firstHalf, secondHalf] = [curr.split('').slice(0, curr.length / 2), curr.split('').slice(curr.length / 2)];
    return sum + priority.indexOf(firstHalf.find(x => secondHalf.includes(x)));
}, 0);
...
let solution2 = input.reduce((sum, curr, index, arr) => {
    if (index % 3 !== 0) {
        return sum;
    }
    const [first, second, third] = [curr.split(''), arr[index + 1].split(''), arr[index + 2].split('')];
    return sum + priority.indexOf(first.find(x => second.includes(x) && third.includes(x)));
}, 0);
